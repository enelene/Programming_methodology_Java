...
public class FacePamphletDatabase implements FacePamphletConstants {
	...
	public void addProfile(FacePamphletProfile profile) {
		...
		if (!allProfiles.containsValue(profile)) {
			allProfiles.put(name, profile);
		} else {
			FacePamphletProfile old = allProfiles.get(name);
			allProfiles.replace(name, old, profile);
		}
	}
	...
	public void deleteProfile(String name) {
		if (allProfiles.containsKey(name)) {
			...
			for (String i : allProfiles.keySet()) {
				FacePamphletProfile next = getProfile(i);
				if (next.removeFriend(name)) {
					next.removeFriend(name);
				}
			}
		}
	}
	...
	/**
	 * creates hashMap, key is profiles owners name, value is facePamphletProfile
	 */
	private HashMap<String, FacePamphletProfile> allProfiles = new HashMap<>();
